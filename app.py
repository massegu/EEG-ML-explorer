...
selected_channels = st.sidebar.multiselect(
    "Canales a usar",
    options=available,
    default=default if default else available
)
...
with tab2:
    ...
    ncomp = st.slider("N componentes", min_value=1, max_value=max_comp, value=min(3, max_comp))
    ...
    with colL:
        ...
        st.dataframe(
            df_table.style
            .format("{:.3f}" if log_table else "{:.2e}")
    ...
    with colR:
        ...
        if mode == "lineal":
            ...
            st.info("**Log10**: comprime rangos grandes. Útil si pocos canales dominan el mapa.")
        else:
        ...
        if len(chs) < 4:
            st.warning(f"No hay suficientes canales con coordenadas para topomap: {len(chs)} / {len(eeg.ch_names)}")
...
with tab3:
    ...
    with colA:
        win_sec = st.number_input("Ventana (s)", min_value=0.5, max_value=30.0, value=2.0, step=0.5, key="ml_win")
    with colB:
        step_sec = st.number_input("Paso/solape (s)", min_value=0.1, max_value=30.0, value=1.0, step=0.1, key="ml_step")
    with colC:
        nperseg = st.number_input("Welch nperseg", min_value=64, max_value=4096, value=512, step=64, key="ml_nper")

    if step_sec > win_sec:
        st.warning("Paso mayor que ventana: tendrás pocas ventanas. Normalmente step <= win.")
    ...
    use_tw = st.checkbox("Añadir traveling-wave metrics como features", value=False, key="ml_use_tw")
    if use_tw:
        ...
        tw_names = ["tw_kmag_mean", "tw_speed_mean", "tw_kx_mean", "tw_ky_mean", "tw_kmag_std"]
        X_feat = np.hstack([X_feat, tw_feats])
        feat_names = feat_names + tw_names
        st.write("X_feat + TW:", X_feat.shape)

    # --- Spatial features (opcional) ---
    use_spatial = st.checkbox("Añadir spatial features (AP/LR/GFP/CoM)", value=False, key="ml_use_spatial")
    if use_spatial:
        ...
        if S.shape[1] == 0:
            st.warning("No se pudieron calcular spatial features (faltan canales típicos o coords).")
        else:
            X_feat = np.hstack([X_feat, S])
            feat_names = feat_names + s_names
            st.write("X_feat + spatial:", X_feat.shape)
            st.caption(f"Spatial features calculadas sobre {target_band} (mapa canal×banda por ventana).")

    # --- Tipo de tarea ---
    task = st.selectbox("Tipo de tarea", ["classification", "regression"], key="ml_task")
    ...
    label_mode = st.selectbox(
        "Cómo definir y",
        ["demo (primera mitad=0, segunda mitad=1)", "manual (pegar lista)", "manual por intervalos (t0,t1,label)"],
        key="ml_label_mode"
    )
    ...
    if label_mode.startswith("demo"):
        ...
        st.caption("Demo: útil solo para comprobar que el pipeline funciona (no es ciencia).")

    elif label_mode.startswith("manual (pegar lista)"):
        st.caption("Pega una etiqueta por ventana (mismo número que ventanas), separadas por comas.")
        y_text = st.text_area("y", value="", height=120, key="ml_y_text")
        if y_text.strip():
            y = np.array([s.strip() for s in y_text.split(",") if s.strip() != ""], dtype=object)

    else:
        st.caption("Pega intervalos: una línea por intervalo -> t0,t1,label (segundos). Ej: 0,30,rest ; 30,60,task")
    ...
    if y is not None and len(y) == len(t_centers) and task == "classification":
        ...
        plt.plot(t_centers, y_num, marker=".", linestyle="none")
        plt.xlabel("Tiempo (s)")
        plt.ylabel("Clase (index)")
        plt.title("Etiquetas por ventana (debug)")
        st.pyplot(fig)
    ...
    if st.button("Evaluar modelos", key="ml_eval"):
        ...
        if len(y) != X_feat.shape[0]:
            st.error(f"y ({len(y)}) debe tener la misma longitud que X_feat ({X_feat.shape[0]}).")
            st.stop()
        ...
        if task == "regression":
            ...
            if len(set(y.tolist())) < 2:
                st.error(f"Necesitas al menos 2 clases. Solo hay: {set(y.tolist())}")
                st.stop()
            ...
            cv = min(5, min_class)
            if cv < 2:
                st.error(f"No hay suficientes muestras por clase para CV. Recuentos: {dict(zip(unique, counts))}")
                st.stop()
...
with tab4:
    ...
    if montage_mode.startswith("standard"):
        ...
        text = st.text_area("Pega tu CSV de canales/coords", value=sample, height=140, key="tw_custom_csv")
    ...
    with cR:
        if st.button("Reset TW", key="tw_reset"):
            ...
            st.info("Resultados TW borrados. Pulsa 'Calcular traveling waves' para recalcular.")
    with cC:
        calc = st.button("Calcular traveling waves", key="tw_calc")

    if calc:
        if len(coords) < min_needed:
            st.error(f"Necesitas al menos {min_needed} canales con coordenadas. Ahora: {len(coords)}.")
            st.stop()
    ...
    if "tw_out" not in st.session_state:
        st.info("Pulsa **Calcular traveling waves** para generar resultados y luego usa el slider/Play.")
        st.stop()

    out = st.session_state["tw_out"]
    coords_used = st.session_state.get("tw_coords_used", coords)
    band_name_used, lo_used, hi_used = st.session_state.get("tw_band_used", (band_name, lo, hi))
    ...
    smooth_win = st.slider("Ventana suavizado (frames)", 1, 61, 11, step=2, key="tw_smooth_win")
    ...
    with cS1:
        scale_mode = st.selectbox("Escalar flecha por", ["constante", "|k|", "speed_proxy"], key="tw_scale_mode")
    with cS2:
        base_len = st.slider("Longitud base flecha", 0.01, 0.20, 0.06, key="tw_base_len")
    ...
    info_u = mne.create_info(ch_names=used, sfreq=eeg.sfreq, ch_types=["eeg"] * len(used))
    mont_u = mne.channels.make_dig_montage({ch: coords_used[ch] for ch in used}, coord_frame="head")
    ...
    with c1:
        ...
        try:
            ret = mne.viz.plot_topomap(phi_t, info_u, axes=ax, show=False, cmap="RdBu_r", vlim=(-np.pi, np.pi))
        except TypeError:
            ret = mne.viz.plot_topomap(phi_t, info_u, axes=ax, show=False, cmap="RdBu_r", vmin=-np.pi, vmax=np.pi)
        im = ret[0] if isinstance(ret, tuple) else ret
    ...
    with c2:
        ...
        axv.quiver(cx, cy, dx * L, dy * L, angles="xy", scale_units="xy", scale=1.0, width=0.010)
    ...
    ph_metrics.caption(f"t={t_vis:.2f}s | θ={theta:.2f} rad | |k|={kmag:.3g} | speed={spd:.3g}")
    ...
    if st.button("Generar GIF", key="tw_make_gif"):
        ...
        for jj in range(start_i, end_i + 1, int(gif_stride)):
            ...
            axg.quiver(cx, cy, dxx * L, dyy * L, angles="xy", scale_units="xy", scale=1.0, width=0.010)
